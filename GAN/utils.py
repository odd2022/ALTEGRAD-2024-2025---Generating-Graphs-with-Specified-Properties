...
def preprocess_dataset(dataset, n_max_nodes, spectral_emb_dim, feature_extractor="extract_numbers"):
    ...
    if dataset == 'test':
        ...
        if os.path.isfile(filename):
            ...
            for line in fr:
                ...
                feats_stats = extract_feats(desc, feature_extractor=feature_extractor)
                feats_stats = torch.FloatTensor(feats_stats).unsqueeze(0)
                data_lst.append(Data(stats=feats_stats, prop=prop, filename = graph_id))
            fr.close()                    
        ...
        if os.path.isfile(filename):
            ...
            for fileread in tqdm(files):
                ...
                file = open(fstats, "r")
                desc = file.read()
                prop = extract_feats(desc, feature_extractor="extract_numbers", file=fstats)
                feats_stats = extract_feats(desc, feature_extractor=feature_extractor, file=fstats)
                feats_stats = torch.FloatTensor(feats_stats).unsqueeze(0)
                file.close()
                data_lst.append(Data(x=x, edge_index=edge_index, A=adj, stats=feats_stats, prop=prop, filename = filen))
            torch.save(data_lst, filename)
            print(f'Dataset {filename} saved')
    return data_lst
